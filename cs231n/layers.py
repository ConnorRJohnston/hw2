...
def svm_loss(x, y):
    ...
    ###########################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    margins = np.maximum(0, x - x[np.arange(x.shape[0]), y][:, np.newaxis] + 1)
...
def softmax_loss(x, y):
    ...
    ###########################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    probabilities = np.exp(x - np.max(x, axis=1, keepdims=True)) / np.sum(np.exp(x - np.max(x, axis=1, keepdims=True)), axis=1, keepdims=True)
    loss = -np.sum(np.log(probabilities[np.arange(x.shape[0]), y])) / x.shape[0]
